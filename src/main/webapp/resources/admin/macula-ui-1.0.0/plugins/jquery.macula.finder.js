...
(function($, undefined) {
	...
	$.maculafinder = function(code, options) {
		...
		var $rside = $('#dlg-side-r').size() ? $('#dlg-side-r') : $('#side-r');
		// 高级查询按钮
		...
		eventBindingElement.bind(
				...
				function(e, data) {
					PageData.dataContent = data.content;
		...
		// 增加处理Dialog状态下的选择
		var choiceAction = $('#finder-' + 'action-choice' + '-' + code);
		if (choiceAction.size()) {
			choiceAction.bind('click', function(e) {
				var pkey = choiceAction.attr('pkey'), result = [];
				$(PageData.dataContent).each(function() {
					if ($.inArray('' + this[pkey],	PageData.selectedItems) > -1) {
						result.push(this);
					}
				});
