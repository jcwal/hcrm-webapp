...
<@layout.ajaxContent title="${viewModel.schema.title} -[$Revision: 4782 $]">
	...
	<div id="side-r-foot">
		<div class="table-action" style="height:auto;padding:0;margin:0;line-height:normal">
	    	<button class="btn btn-primary" id="filter-submit-${schema.code}" type="button"><span><span>立即筛选</span></span></button>
	    	<button class="btn btn-secondary" id="filter-reset-${schema.code}" type="button"><span><span>清除条件</span></span></button>
	    </div>
	</div>   
	<script>
		(function(){
