<@layout.ajaxContent title="<#if doctorId?exists>编辑医生信息<#else>新增医生信息</#if> -[$Revision: 3569 $]" scripts="admin/macula-trial/doctor/edit.js">
	<#assign code="edit-doctor" />
	<@layout.content_main>	
	<div id="page-${code}" class="tableform">
		<form id="form-${code}" item-id="${doctorId?if_exists}" action="${base}/admin/macula-trial/doctor/save" method="post">
				...
				<table cellspacing="0" cellpadding="0" border="0">
					<tbody>
						...
						<tr>
							...
							<td>
								<input name="doctor.title" data-bind="value: title" validate="rangelength:[0,20]" style="width: 180px;" />
							</td>
							<th><label>科室：</label></th>
							<td>
								<input name="doctor.department" data-bind="value: department" validate="rangelength:[0,20]" style="width: 180px;" />
							</td>									
						</tr>
						<tr>
							...
							<td>
								<input type="text" name="doctor.mobile" data-bind="value: mobile"  style="width: 180px;"  validate="rangelength:[0,20]"/>
							</td>							
						</tr>
					</tbody>
				</table>
